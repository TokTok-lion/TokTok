...
on-screen size, and written as WebP (alpha preserved).

Run scripts/build-manifest.py afterwards to regenerate lib/art.ts — the
manifest is built by scanning public/art so that artwork cut from the design
deck (prepare-scenes.py) is included too.
...
# Remove only this script's own outputs. public/art also holds artwork cut
# from the design deck by prepare-scenes.py, which must survive a re-run.
for _stem, _cap in MAP.values():
    for ext in ('.webp', '.png'):
        _p = os.path.join(DST, _stem + ext)
