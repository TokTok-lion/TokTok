 ...
 * 녹음 보관기간 — **30일**.
 *
 * 임시값이 아니다. 첫 기관과 합의한 값이고(2026-08-13), 명세의 허용 범위
 * (center.ts 의 RETENTION_BOUNDS.voice = 30~365일) 중 가장 짧은 쪽이다.
 * 원음성은 이 서비스에서 가장 민감한 자료라 더 길게 둘 이유가 없다.
 *
 * 줄일 수는 없다 — 30일이 하한이다. 늘리려면 기관과 다시 합의해야 하고,
 * 그때는 이 상수가 아니라 기관별 값이 되어야 한다(곡이 이미 그렇게 되어
 * 있다: tenants.song_retention_days).
 *
 * 지금 이 값이 지키는 것은 **기기 안의 녹음**뿐이다. 녹음은 기관 저장소로
 * 올라가지 않는다. 서버 보관이 생기면 같은 30일을 그쪽에도 걸어야 하고,
 * 두 곳이 어긋나면 "지웠다"는 말이 거짓이 된다.
 *
 * 실제로 지우는 것은 prune 이다(loadRecording 이 부를 때마다 돈다). 화면에는
 * 남은 날수가 뜬다(components/StoredAudio).
 */
