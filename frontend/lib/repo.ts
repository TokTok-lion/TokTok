 ...
 * 약 237원이었다. 지금은 Treblo 를 쓰고 곡당 약 $0.05 — 70원쯤이다.
 *
 * 구독을 나눠 쓰는 방식과 달리 만든 만큼만 나가므로, 이 값은 실제 청구액에
 * 꽤 가깝다. 그래도 화면에서는 "어림값"이라고 못 박는다 — 환율과 세금이
 * 남아 있고, 청구서가 아니라 가늠자다.
 */
const KRW_PER_SONG = Number(process.env.NEXT_PUBLIC_KRW_PER_SONG) || 70;
