 ...
 */
export function cueFractions(cues: number[], total: number): number[] {
...
/**
 * 지금이 곡의 어디쯤일 때 몇 번째 줄인가.
 *
 * starts 는 줄 수 + 1 개다(마지막 칸이 곡의 끝). 뒤에서부터 훑어 처음으로
 * 지나온 자리를 짚는다.
 */
export function lineAtFraction(starts: number[], f: number): number {
