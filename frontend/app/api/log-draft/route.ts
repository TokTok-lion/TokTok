...
'관찰된 행동'에 자주성·공생성이 붙어 옵니다:
- 그 말을 그대로 살려 씁니다. 복지관이 실제로 쓰는 평가 축입니다.
  예) "스스로 이야기를 먼저 꺼내셨다", "다른 어르신 말씀에 맞장구치셨다"
- 다만 본 것만 씁니다. "자주성이 향상되었다" 같은 판정은 쓰지 마십시오 —
  향상 여부는 여러 회기를 놓고 사람이 판단하는 것입니다.
...
export async function POST(req: Request) {
  ...
  const user = [
    ...
    '',
    /*
     * 앞에 '자주성:' '공생성:' 이 붙어 온다(app/session/log). 복지관이 실제로
     * 재는 축이라 초안이 그 말로 적히면 보고서에 그대로 옮길 수 있다.
     */
