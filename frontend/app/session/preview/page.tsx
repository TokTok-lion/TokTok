 ...
 * 보였다.
 *
 * 세 버전을 진짜로 만들려면 곡값이 세 배로 든다. 그래서 고르는 화면을 없애고,
 * 만들어진 그 한 곡을 실제로 들려주는 화면으로 바꿨다. 여기서 정할 것은
 * "이 곡으로 갈까요, 다시 만들까요" 하나다.
 *
 * 다만 값이 세 배라는 전제는 다시 볼 만하다. Suno 는 한 번 만들 때 **트랙을
 * 두 개** 내주는데 앱은 첫 번째만 쓰고 두 번째를 버린다(music-apiframe).
 * 이미 값을 치른 트랙이므로, 두 가지 중에 고르는 화면은 요금을 더 쓰지 않고도
 * 되살릴 수 있다. 셋이 아니라 둘이면 된다.
 */
