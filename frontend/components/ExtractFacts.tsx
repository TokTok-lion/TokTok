  ...
  const run = async () => {
    ...
    try {
      ...
      if (!json.facts.length) {
        /*
         * 빈손으로 돌아오는 길이 둘인데 오래 한 문장으로 덮여 있었다.
         *
         *   가) 정말 옮길 말씀이 없었다 — 인터뷰가 짧거나 대답이 짧으셨다.
         *   나) 뽑기는 했는데 **근거를 못 맞춰 전부 버렸다**. 라우트가 그 수를
         *       dropped 로 돌려주는데, 화면은 그 값을 사실이 있을 때만 읽었다.
         *
         * 둘은 복지사가 할 일이 다르다. 가)는 더 여쭈어야 하고, 나)는 전사를
         * 손보고 다시 뽑으면 된다. 같은 말로 덮으면 있지도 않은 문제를 찾느라
         * 전사만 몇 번 다시 읽다 포기한다 — 실제로 그렇게 보고를 받았다.
         *
         * 짧은 인터뷰에서 나)가 자주 나는 이유가 있다. 줄이 몇 개 없으면 모델이
         * 근거로 댈 어르신 줄도 몇 개 없어서, 복지사 질문 줄을 근거로 대기
         * 쉽다. 그 줄은 출처가 될 수 없으므로(원칙 1) 통째로 버려진다.
         */
        const words = s.transcript
          .filter((l) => !l.example && l.speaker !== 'worker')
          .reduce((n, l) => n + l.text.trim().length, 0);

        setError(
          json.dropped
            ? `말씀 ${json.dropped}개를 뽑았는데, 어느 대목에서 나온 말씀인지 ` +
              '확인되지 않아 모두 버렸어요. 출처 없는 이야기는 남기지 않습니다. ' +
              '전사 교정에서 어르신 말씀에 이름표가 제대로 붙어 있는지 보시고 ' +
              '다시 뽑아 주세요.'
            : words < 60
              ? '아직 옮길 말씀이 많지 않아요. 어르신 말씀이 ' +
                `${words}자쯤 담겼는데, 이야기로 남기려면 조금 더 여쭤 주세요. ` +
                '인터뷰 화면의 「다음 질문」으로 두세 가지만 더 나누셔도 됩니다.'
